...
DIALOGUE DEFAULT (this skill):
Do not invent spoken dialogue unless the user provided quoted speech.
If the user included dialogue in quotes, weave those exact words into the prose with attribution and delivery.
If there is no user dialogue, use ambient + action sound only — no spoken words.
