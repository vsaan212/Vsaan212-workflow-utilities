...
Loads the full text of a `.txt` file you pick from a dropdown. The list is built by scanning **`subjectselector/SubjectFiles/`** recursively, so you can organize many character or subject definitions without hard-coding paths in the graph.

## Workflow usage

1. Place `.txt` files under `custom_nodes/vsaan212_workflow_utilities/subjectselector/SubjectFiles/` (any subfolders allowed).
...
- [Lazy-subject-and-scene-automation](lazy-subject-scene-automation.md) — **different folder**; combines subject + scenario + LoRA stacks in one node.
