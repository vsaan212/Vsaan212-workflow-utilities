...
Stores reusable prompt snippets in **`ComfyUI/lazynodes/lazy_prompts.json`**. You can name prompts, edit text in the graph, save, clone, and delete without leaving ComfyUI. The file sits outside `custom_nodes/` so pack updates do not wipe it. On first startup an empty `{}` library is created if the file is missing (or copied from an old pack copy if one still exists).
...
4. **Save** — writes the current name + text to `ComfyUI/lazynodes/lazy_prompts.json`.
...
- Back up `ComfyUI/lazynodes/lazy_prompts.json` if you rely on it across machine moves. Pack updates no longer delete it.
